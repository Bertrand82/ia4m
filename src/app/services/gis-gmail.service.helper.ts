...
import { onChangeEmailSelected, Updatable } from './UpDatable';
import { ComponentEmailDetail } from '../component-email-detail/component-email-detail';
@Injectable({
  providedIn: 'root',
})
export class GisGmailServiceHelper implements OnInit {
  ...
  fetchMessageAndProcess(idMessage: string) {
    this.gmail
      ...
      .then((mg) => {this?.updatable?.updateView(); this.processMessageDetailsByGemini(mg)})
      .catch((err) => console.error('bg processMessage id :' + idMessage + "  err:", err));
  }
  ...
  processMessageDetailsByGemini(msgG: any): any {

    console.log('processMessageDetails msgG:', msgG);

    const index = this.messages.findIndex((m) => m.id === msgG.id);
    ...
    this.messages[index] = bgMessage;
    console.log('bg B processMessageDetails msgG.id:', msgG.id, ' message=', bgMessage);

    // build prompt
    if (bgMessage.isConsistent()) {
      console.log('bg C processMessageDetails message is consistent id=', bgMessage.id, "message:", bgMessage);
    } else {
      console.error('bg C bg Message non consistent, skipping Gemini analysis id=', bgMessage);
      return;
    }
    ...
    this.gemini.generateContent(prompt, reponseAnalyseGmail).subscribe({
      next: (res: { geminiData: { error: any; usageMetadata: any; candidates: any[]; }; }) => {
        console.log('responseRequestGemini msg A' + msgG.id + ' res:', res);
        console.log(
